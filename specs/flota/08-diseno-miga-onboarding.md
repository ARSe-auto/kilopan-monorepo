...
- Reparto del paquete en ACs: **AC-FMIG-18** (cerrado) deja sembrados B y C y el
  ...
  **AC-FMIG-26** la operación de B (rutas, manifiestos con DTE, andén, reintento,
  ...
  A/B/C; **AC-FMIG-23** el test de fixture que consume esa memoria sobre el
  DENOMINADOR, y **AC-FMIG-28** el numerador y el `eevd` completo, que siguen
  bloqueados por la Pregunta al dueño 4 de la spec 04.
...
- [x] (P1) DPA en términos del tenant (§3.E1.15, §7.8 — obligación E1 sin módulo asignado por el maestro; queda aquí, dueño del panel admin white-label y del wizard, hito g): el DPA existe como artefacto VERSIONADO del repo (test CI grep-able de existencia y de secciones mínimas: partes, objeto del tratamiento, encargado/responsable, subencargados, medidas de seguridad, devolución/supresión al término y portabilidad `pg_dump` del §2 métrica 7), se sirve dentro de los términos del tenant sin CSS libre ni build por cliente (§5.1), es alcanzable desde el panel admin dentro de los 2 niveles de profundidad (§5.1) y su versión vigente queda registrada por tenant; la aceptación por el `admin_tenant` escribe `audit_trail` con la versión aceptada (§4.6) y con rol distinto de `admin_tenant` ⇒ 403 y 0 filas (§5.4); textos es-CL, cero strings visibles en inglés (§0). El TEXTO legal y el MOMENTO exacto de la aceptación (paso del wizard vs panel admin) quedan BLOQUEADOS por la Pregunta al dueño 12 — esta spec no los inventa; el AC cierra hoy con la parte estructural y se completa al responderse — oráculo: CI [AC-FMIG-22]
  PROBADO: `packages/miga/src/dpa.test.ts` (7 secciones mínimas, pg_dump, base de licitud
  ejecución-de-contrato, cero cáscaras, versión no vacía, profundidad ≤2) +
  `db/migraciones-flota/tenant/0067_dpa_del_tenant.sql` (tabla `dpa_aceptaciones` con
  `auditar()`) + `apps/flota/e2e/dpa.spec.ts` (5/5 verde: GET sirve las 7 secciones sin
  aceptación previa, POST escribe `audit_trail` con la versión, idempotencia sobre la misma
  versión, pantalla `/panel/dpa` sirve y deja aceptar, rol distinto de `admin_tenant` no ve
  el botón). `check.sh --full --app=flota`: VERDE (verde-20260816-065445).
- [x] (P1) Test de fixture de la EEVD del DONE-software (§10) sobre el DENOMINADOR — oráculo que el maestro exige y que hasta ahora no tenía dueño: el valor esperado HARDCODEADO, calculado UNA vez a mano al escribir el seed y con su memoria de cálculo versionada junto a los seeds A y B (sección 7, AC-FMIG-27), se compara contra la vista `eevd_semanal` (creada por el módulo 02, AC-FVEH-20) computada sobre las BDs sembradas desde `tenant_template`; discrepancia ⇒ gate rojo, con mutante VIVO que lo demuestra. El DENOMINADOR (vehículos-día con turno abierto, §2) es computable desde ya y es lo que este AC cierra. El NUMERADOR (paradas tipo `entrega` con `estado='done'`, resultado `exito|parcial` y ≥1 fila válida en `evidence`) y el valor `eevd` completo se partieron a AC-FMIG-28 —igual que AC-FMIG-10 hizo con AC-FMIG-24— porque siguen BLOQUEADOS por la Pregunta al dueño 4 de la spec 04, y este AC jamás elige por cuenta propia qué fila de `evidence` escribe la entrega feliz — oráculo: CI [AC-FMIG-23] — `db/flota/suite-bd/eevd-fixture.test.mjs` siembra A y B enteros desde `tenant_template` y COMPARA (nunca recalcula: un test que recalcula está comparando la vista contra sí misma) la suma de `vehiculos_dia` de `eevd_semanal` contra `EEVD_ESPERADO_A.denominador` = 1 y `EEVD_ESPERADO_B.denominador` = 2 de `db/flota/seeds/eevd-esperado.mjs`. La suma va sobre TODAS las filas de la vista y no sobre una semana de guion porque `abrirTurno` no recibe timestamp en ninguno de los dos seeds: lo que la memoria congela es la CANTIDAD de vehículos-día, caiga en la semana que caiga. El caso de rebote es un mutante vivo en los DOS tenants: una fila de `turnos` plantada 30 días atrás —otra semana, y `periodo` cerrado a la hora para no chocar con el EXCLUDE `turnos_sin_solape` del turno vivo— sube el denominador real a 2 y a 3, la comparación contra la memoria se pone ROJA, y el `rollback` (obligado: `turnos` es append-only, §7.4) la devuelve a verde; sin ese mutante los dos tests serían verdes también con una vista que no leyera `turnos`.
- [ ] (P1) El NUMERADOR de la EEVD y el valor `eevd` completo contra la memoria de cálculo — mitad partida de AC-FMIG-23, que cerró sobre el denominador: el test de fixture aserta `entregas_con_evidencia` y el `eevd` de `eevd_semanal` contra el valor HARDCODEADO de `db/flota/seeds/eevd-esperado.mjs`, y deja de estar `skip` para entrar al gate. BLOQUEADO por la Pregunta al dueño 4 de `specs/flota/04-pod-offline-sync.md`: el numerador del §2 exige ≥1 fila válida en `evidence` para la parada `entrega` con `estado='done'` y resultado `exito|parcial`, pero el enum de tipos del §4.6 no tiene ninguno capturable en la entrega feliz de 2 acciones sin foto ni firma (§5.2 F4), así que `entrega.con_evidencia` todavía no está en `EVENTOS_OPERACION` y el numerador de ambos tenants es 0 por AUSENCIA DE CATÁLOGO, no por una elección de este test — elegir acá qué fila de `evidence` escribe la entrega feliz es exactamente lo que la spec 04 prohíbe hacer sin el dueño. Se cierra en el mismo commit que responda la Pregunta 4, haga emitir el evento y actualice la derivación de la memoria de cálculo — oráculo: CI (condicionado a la Pregunta 4 de la spec 04) [AC-FMIG-28]
