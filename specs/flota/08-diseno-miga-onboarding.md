...
- [ ] (P1) PARCIAL — tenants B y C sembrados y el caso de rebote de FILA CRUZADA construido, todo verde. HECHO: `db/flota/seeds/tenant-b.mjs` (identidad + vertical panadería, tema propio, terminología al máximo largo permitido 7/7, 2 EV48 de 90 bultos/41.860 Wh, 4 panaderías cliente con RUT sintético); `db/flota/seeds/tenant-c.mjs` + `suite-bd/seed-tenant-c.test.mjs` (modo `mi_flota`; 1 EV48 con ficha EV completa —autonomía nominal 185 km, el PISO del rango real del Anexo A, jamás los 305 km del folleto que el maestro prohíbe planificar—; 1 chofer real por invitación + aprobación en 1 toque; la empresa IMPLÍCITA como ÚNICA empresa, que es la contracción de mi_flota en la capa de datos: sin contratantes no hay portal ni liquidación por cliente; día de 3 encargos propios con sus 3 PODs aterrizados y el tablero F1 respondiendo con SOC capturado, sin `falta`); `db/flota/seeds/centinelas.mjs` + `db/flota/centinelas-de-seeds.test.mjs` (el registro de los 3 centinelas y la propiedad que los hace servibles como oráculo —disjuntos y ninguno subcadena de otro—, con 5 mutantes, en el gate RÁPIDO); y `db/flota/suite-bd/seeds-fila-cruzada.test.mjs`, el oráculo del caso de rebote: barre la huella de TODAS las columnas de texto descubiertas del CATÁLOGO de cada base (§9.3.2 centinela 2: el 404 no alcanza, hay que mirar la BD del vecino), con control positivo, la propiedad, y una fila cruzada plantada a mano que lo pone ROJO y cuyo retiro lo devuelve a verde. FALTA (AC sigue abierto): el tenant A completo —bloqueado en parte: `otd_comprometido_pct` de la farmacia no existe como columna en ninguna migración y el motor jamás toca `db/migraciones*/` (AGENTS.md), así que necesita sesión supervisada de esquema—; de B, las 2 rutas de madrugada (12 y 9 paradas), los manifiestos firmados con DTE, el encargo de andén, el reintento y el cierre con ecuación cuadrada; de A, la agenda con recarga AC nocturna, las excepciones (no-entrega/parcial/devolución/descuadre) y las liquidaciones cerrada/disputada/pagada; la memoria de cálculo del valor EEVD esperado de A y B (su test de fixture es AC-FMIG-23); y el e2e HTTP del camino dorado A/B/C, del que el test de arriba es la mitad de BD. Seeds de los 3 tenants exactamente según §10 — A «e-auto DaaS» (3 EV48 de 90 bultos/41.860 Wh, 6 usuarios, 3 empresas con sus conceptos y `otd_comprometido_pct=95` en la farmacia, 25 destinos, agenda con recarga AC nocturna, 1 no-entrega + 1 parcial + 1 devolución + 1 descuadre clasificado, liquidaciones cerrada/disputada/pagada), B «Rutapan» (2 EV48, 4 panaderías, rutas de madrugada de 12 y 9 paradas, manifiestos con DTEs, 1 encargo de andén, 1 reintento, cierre cuadrado, terminología al máximo largo, tema propio) y C «Demo Mi Flota» (mi_flota, navegación contraída, 1 día con PODs y semáforo) — con cadenas centinela ÚNICAS por tenant, RUTs sintácticamente válidos pero irreales (cero datos personales reales, §7.8) y la memoria de cálculo del valor EEVD esperado para el test de fixture contra `eevd_semanal` (§10 DONE-software); caso de rebote: e2e del camino dorado A/B/C corre sobre estos seeds y una fila cruzada entre tenants ⇒ rojo (§10, §9.3.2) — oráculo: CI [AC-FMIG-18]
