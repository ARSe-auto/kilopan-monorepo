...
- [ ] (P1) PARCIAL — ver `db/flota/seeds/tenant-b.mjs` + `db/flota/suite-bd/seed-tenant-b.test.mjs` (verde): tenant B «Rutapan» tiene identidad+vertical panadería, tema propio, terminología al máximo largo permitido (7/7 términos), 2 EV48 (90 bultos/41.860 Wh) y 4 panaderías cliente con RUT sintético + centinela único `CENTINELA-RUTAPAN-B7f2`. FALTA (AC sigue abierto): rutas de madrugada 12/9 paradas, manifiestos+DTE, encargo de andén, reintento, cierre cuadrado — todo B; el tenant A completo (bloqueado en parte: `otd_comprometido_pct` de la farmacia no existe como columna en ninguna migración — requiere sesión supervisada de esquema, AGENTS.md); el tenant C completo; la memoria de cálculo EEVD; y el e2e del camino dorado A/B/C con el caso de rebote de fila cruzada (§9.3.2) — oráculo real de este AC, no construido. Seeds de los 3 tenants exactamente según §10 — A «e-auto DaaS» (3 EV48 de 90 bultos/41.860 Wh, 6 usuarios, 3 empresas con sus conceptos y `otd_comprometido_pct=95` en la farmacia, 25 destinos, agenda con recarga AC nocturna, 1 no-entrega + 1 parcial + 1 devolución + 1 descuadre clasificado, liquidaciones cerrada/disputada/pagada), B «Rutapan» (2 EV48, 4 panaderías, rutas de madrugada de 12 y 9 paradas, manifiestos con DTEs, 1 encargo de andén, 1 reintento, cierre cuadrado, terminología al máximo largo, tema propio) y C «Demo Mi Flota» (mi_flota, navegación contraída, 1 día con PODs y semáforo) — con cadenas centinela ÚNICAS por tenant, RUTs sintácticamente válidos pero irreales (cero datos personales reales, §7.8) y la memoria de cálculo del valor EEVD esperado para el test de fixture contra `eevd_semanal` (§10 DONE-software); caso de rebote: e2e del camino dorado A/B/C corre sobre estos seeds y una fila cruzada entre tenants ⇒ rojo (§10, §9.3.2) — oráculo: CI [AC-FMIG-18]
