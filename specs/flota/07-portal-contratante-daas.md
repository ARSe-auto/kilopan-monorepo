...
- [x] (P1) Centinela 11 — conmutación aditiva, jamás destructiva: sobre un tenant con
      ...
      (`pnpm check:full --app=flota`). Una SEGUNDA suite, `db/flota/suite-bd/centinela-11-portal.test.mjs`, cubre el mismo AC por otro ángulo: barrido dinámico por `pg_class`/`pg_constraint` y un caso explícito de que `eventos` SÍ crece, para que el oráculo no acepte un «=» disfrazado de «≥». Las dos corren en el gate.
