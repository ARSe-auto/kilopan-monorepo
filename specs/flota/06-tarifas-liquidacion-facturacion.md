...
- [ ] (P1) Dinero invisible — APLICAR la política a las tablas de montos del módulo (§4.8, §9.3.10; mitad partida de [AC-FTAR-09], que ya cerró el patrón y su prueba con el rol real): `select aplicar_rls_de_dinero(…)` sobre las cinco tablas de montos que hoy NO la llevan —`tarifas` (`precio_clp`), `tarifa_zonas` y `tarifa_recargo_horario` (`monto_clp`), `liquidacion_lineas` (`monto_clp`, `precio_base_clp`, `modificadores_clp`) y `parametros` (`tarifa_kwh_clp`, `precio_diesel_litro_clp`)—, hoy con `aplicar_rls_de_empresa()` (confinamiento del `cliente`, 0040) pero sin la RESTRICTIVE del chofer: `energy_entry` es la ÚNICA tabla de montos del árbol que la lleva. Con la política puesta, el `bag_eq` de `db/flota/pgtap/0032_dinero_invisible.sql` se pone rojo hasta que la tabla salga de su lista congelada de pendientes, y las tres aserciones de forma de esa misma suite pasan a cubrirlas sin tocar el test — BLOQUEADO por DDL de sesión supervisada: el motor no escribe migraciones. Y trae UNA decisión que no es mecánica: `parametros` es una tabla de UNA fila con configuración que el terreno necesita leer (no solo montos), así que una RESTRICTIVE de fila la dejaría entera invisible para el chofer; el §4.8 pide esconder el MONTO, no la configuración, de modo que ahí el instrumento probablemente sea `revoke select (tarifa_kwh_clp, precio_diesel_litro_clp)` a nivel de COLUMNA o una vista sin esas dos, y esa elección es del dueño del esquema, no del motor — oráculo: CI [AC-FTAR-17]
