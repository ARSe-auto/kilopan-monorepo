...
export default function PortalLiquidaciones() {
  ...
  if (id === null) return <ListaLiquidaciones />;
  return <DetalleLiquidacion id={id} />;
}
...
function DetalleLiquidacion({ id }: { id: string }) {
  ...
  if (error) {
    return (
      <main data-testid="portal-liquidacion-detalle">
        <h1 style={titulo}>Liquidación</h1>
  ...
  return (
    <main data-testid="portal-liquidacion-detalle">
      <a data-testid="volver-liquidaciones" href="/cliente/liquidaciones" style={{ ...pie, color: superficie.textoDim }}>
          ...
          {liquidacion.lineas.map((linea) => (
            <li key={linea.id} data-testid="linea-liquidacion-cliente" data-id={linea.id} style={tarjeta}>
              ...
              {linea.disputa_estado ? (
                <p data-testid="linea-disputada-cliente" style={{ ...pie, margin: 0, color: "#B45309" }}>
                  Disputada{linea.disputa_nota ? `: ${linea.disputa_nota}` : ""}
                </p>
              ) : liquidacion.estado === "cerrada" ? (
                <FormularioDeDisputa lineaId={linea.id} motivos={motivos} alDisputar={() => void cargar()} />
              ) : null}
            </li>
          ))}
        </ul>
...
function FichaDeEvidencia({
  ...
  return (
    <div data-testid="evidencia-pod-cliente" style={{ display: "grid", gap: semantico.espacio.entreControles }}>
      ...
      {d.motivo_etiqueta && (
        <p data-testid="evidencia-motivo-cliente" style={{ ...pie, margin: 0, color: superficie.textoDim }}>
      ...
      {d.capturas.length === 0 ? (
        ...
        >
          {d.capturas.map((captura, i) => (
            <li key={i} data-testid="evidencia-captura-cliente" style={{ ...pie, color: superficie.texto }}>
...
function FormularioDeDisputa({
  ...
  return (
    <div data-testid={`formulario-disputa-${lineaId}`} style={{ display: "grid", gap: semantico.espacio.entreControles }}>
      ...
      {error && (
        <p role="alert" style={{ ...pie, margin: 0, color: "#B91C1C" }}>
          {error}
        </p>
      )}

      <div style={{ display: "flex", gap: semantico.espacio.entreControles }}>
        <BotonPrimario testid="enviar-disputa" disabled={motivoId === "" || enviando} onClick={() => void enviar()}>
