...
export default function FacturarPage() {
  ...
  return (
    <Pantalla
      titulo="Consolidar y facturar"
      bajada="Cada despacho ya salió con su guía. Aquí las agrupas en una factura para cobrar."
      ancho={620}
    >
      <select
        value={clienteId}
      ...
      {mensaje ? (
        <p role={mensaje.tipo === "error" ? "alert" : "status"} style={{ color: mensaje.tipo === "ok" ? semantico.ok : semantico.error, fontSize: 14 }}>
          {mensaje.texto}
        </p>
      ) : null}
    </Pantalla>
