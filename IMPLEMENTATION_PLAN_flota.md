...
- [ ] (P1) Contracción por modo/entitlement — manifest sin tarifas/liquidación/facturación y endpoints 403 (mitad partida del AC-FTAR-12): BLOQUEADO por DDL (las 4 features de `modo_recorte` nunca se insertaron en la tabla `features` de `control`, así que el snapshot congelado nunca las incluye para ningún modo) y por infraestructura genérica de manifest que son AC-FMIG-09 (spec 08) / AC-FPOR-03 (spec 07), no de este módulo — spec: specs/flota/06-tarifas-liquidacion-facturacion.md [AC-FTAR-18]
- [ ] (P2) `otd_comprometido_pct` smallint NULL CHECK 50–100; fuera de rango rebota; NULL desactiva tarjeta/señal SLA; farmacia 95 la muestra en el camino dorado — BLOQUEADO: la columna no existe en `empresas_cliente` (creada en 0036, sin tocar desde 0039) y agregarla es DDL de sesión supervisada; el consumo TS de lectura ya está listo en semaforo-fixtures/semaforo-daas-sla (AC-FSEM-08) — spec: specs/flota/06-tarifas-liquidacion-facturacion.md [AC-FTAR-13]
...
- [ ] (P1) Registro MANUAL del folio (mitad de mutación partida del AC-FTAR-08): `reference_document(33|39|52|61, folio, emisor)` sobre liquidación `cerrada`, sobre `abierta` ⇒ 422, duplicado ⇒ 422 y 0 filas, folio asociado a SU liquidación — BLOQUEADO: pide DDL (`liquidaciones.reference_document_id` + FK compuesta + índice, o tabla `liquidacion_documento`), que es de sesión supervisada — spec: specs/flota/06-tarifas-liquidacion-facturacion.md [AC-FTAR-16]
- [ ] (P1) Dinero invisible — APLICAR la política a las 5 tablas de montos del módulo (mitad partida del AC-FTAR-09): `aplicar_rls_de_dinero()` sobre `tarifas`, `tarifa_zonas`, `tarifa_recargo_horario`, `liquidacion_lineas` y `parametros` (hoy solo `energy_entry` la lleva); en `parametros` la decisión no es mecánica —esconder el monto sin dejar ciega la configuración del terreno: `revoke select` por COLUMNA o vista— BLOQUEADO: pide DDL, que es de sesión supervisada — spec: specs/flota/06-tarifas-liquidacion-facturacion.md [AC-FTAR-17]
