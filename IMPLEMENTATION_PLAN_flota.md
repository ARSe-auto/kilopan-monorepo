...
- [ ] (P1) Gate AA axe+Lighthouse de la pantalla de parada: 4.5:1/3:1/7:1, targets, foco, 200% sin truncar, cero aria-labels vacíos — spec: specs/flota/04-pod-offline-sync.md [AC-FPOD-23]
  - REVERTIDO el 12-ago-2026 03:20 (era ea5b9b1). Las 5 aserciones de axe estaban bien planteadas —contraste 4.5:1/3:1/7:1, targets, aria-labels, zoom 200%, foco— pero su FIXTURE estaba escrito contra un esquema que no existe: insertaba vehiculos(empresa_id, modelos_json, soc_por_ciento, odometro_km) cuando la tabla real tiene (patente, tipo, capacidad_bultos, bateria_wh, soc, odometro), y lo mismo en turnos, rutas, destinos y encargos. Ninguna de las dos pruebas llegó a mirar un pixel. Ademas el commit agrego @axe-core/playwright a package.json SIN su entrada del lock, que en CI revienta el install antes de correr nada. AL REHACERLO: copiar el fixture de una suite que ya corre en verde (pod-feliz o pod-variantes) en vez de escribirlo de memoria, y comitear package.json y pnpm-lock.yaml en el MISMO commit.
