...
- [x] (P1) Los 4 estados obligatorios de la pantalla de parada (vacío/skeleton/error sin rechazo/sin conexión con contador) — spec: specs/flota/04-pod-offline-sync.md [AC-FPOD-22] — probado: `Estados4Parada` componente cliente que envuelve TarjetaDeEntrega; 4 estados: EstadoVacio (secuencia vacía), EstadoSkeleton (<50ms), EstadoSpinner (>400ms), y EstadoSinConexión (ya en TarjetaDeEntrega con contador real). El candado del snapshot llega en prop y convive sin viajes por parada. Layout.tsx agrega animaciones CSS (pulse/spin). pnpm check VERDE (11/11 pasos).
