...
- [x] (P1) Dinero invisible — la política aplicada a las 5 tablas de montos del módulo (mitad partida del AC-FTAR-09): `aplicar_rls_de_dinero()` sobre `tarifas`, `tarifa_zonas`, `tarifa_recargo_horario` y `liquidacion_lineas`; `parametros` no era mecánica —el dinero convive en la MISMA fila con la configuración del terreno, así que la RLS lo dejaría sin la fórmula del §0 y un `revoke` por columna mira el rol de Postgres, que es uno por tenant— y se resolvió por estructura con la vista `parametros_operativos`, que el terreno ya consume — spec: specs/flota/06-tarifas-liquidacion-facturacion.md [AC-FTAR-17]
  - Probado: migración `0073` + pgTAP `0037` (RLS y las dos políticas en las cuatro tablas; la
    vista sin `tarifa_kwh_clp`/`precio_diesel_litro_clp` y con `reserva_pct`, `factor_consumo`,
    `bultos_max_sin_receptor`). La mitad de aplicación la vigila
    `db/flota/gate-terreno-sin-parametros.mjs`, NUEVO, con 9 mutantes: en `apps/flota/src` toda
    lectura de `parametros` va por la vista salvo los declarados (hoy `servidor/tablero.ts`, del
    operador), lista exacta en las dos direcciones, y exige que alguien lea la vista para que el
    verde no sea vacuo. `src/app/entrega/page.tsx` —la pantalla del chofer— pasó a la vista.
...
- [x] (P1) `posiciones` como CAPTURA del §4.6: tabla en `tenant_template` con tenant_id compuesto, append-only, y los 6 invariantes pgTAP de la spec — spec: specs/flota/09-rastreo-telemetria-export.md [AC-FTEL-02] — probado: migración 0075 (client_uuid único por tenant, precision_m, tz_offset_min, recibida_en, fuente enum, CHECK de caja Chile) y pgTAP 0039 con las seis invariantes en 31 pruebas; `check.sh --full --app=flota` VERDE
- [x] (P1) La posición viaja por el MISMO motor de sync (§4.6): lote con `posiciones` + otros hechos entra 2xx, offline-first sin canal aparte — spec: specs/flota/09-rastreo-telemetria-export.md [AC-FTEL-03] — probado: e2e/rastreo-outbox.spec.ts (offline→online, lote mixto 2xx, idempotencia) y unit gate-capturas-sin-rechazo.test.mjs; cierra el AC-ABIERTO de f8ca264 agregando a estado-de-rastreo.tsx la sección «por sincronizar» con contador real que le faltaba (§5.7); `check.sh --full --app=flota` VERDE
