...
import { GANCHO, siembrasEn, sinComentarios, DONDE_VIVE } from "./gate-seeds-pin-destinatario.mjs";

// Mutantes del chequeo de seeds del gancho `pin_destinatario` [AC-FRUT-20] — §4.9, §5.2 F4.
//
...
test("nombrarlo en un comentario SQL para explicar por qué NO está es legítimo", () => {
  const sql = `-- El tipo 'pin_destinatario' existe en el enum y ningún seed E1 lo siembra.
...
test("nombrarlo en un comentario de JavaScript tampoco rebota", () => {
  assert.deepEqual(siembrasEn(`// en E1 nadie siembra 'pin_destinatario' (§5.2 F4)`), []);
});

test("ni dentro de un bloque /* */, que es donde va la explicación larga", () => {
  const js = `/* El gancho 'pin_destinatario' es de E2.
                 Acá no se siembra. */
              const tipo = "firma";`;
...
test("mencionarlo SIN comillas no es sembrarlo: es hablar de él", () => {
  ...
  assert.deepEqual(siembrasEn("El gancho pin_destinatario queda para E2."), []);
});
...
test("el gancho tiene que seguir declarado en el enum del DDL", () => {
  ...
  // OPUESTO: el §4.9 lo quiere vivo para que encenderlo en E2 sean filas y no una migración.
