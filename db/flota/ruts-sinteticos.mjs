...
VALIDOS["13.444.555-6"] = "el chofer de apps/flota/e2e/rastreo-outbox.spec.ts (AC-FTEL-03), propio y no prestado: la suite abre un turno con GPS mockeado y corta la red, y comparte el tenant `hechos` con `rastreo.spec.ts` (AC-FTEL-01) — necesita su propio RUT para no chocar con duplicate key en `personas_tenant_id_rut_key`. Dígito verificador DERIVADO por módulo 11.";
VALIDOS["14.555.666-K"] = "la gestora de apps/flota/e2e/torre-de-control.spec.ts (AC-FTEL-04), propia y no prestada: la suite comparte el tenant `hechos` con `rastreo.spec.ts`/`rastreo-outbox.spec.ts` (AC-FTEL-01/03) y necesita su propio RUT para no chocar con duplicate key en `personas_tenant_id_rut_key`. Dígito verificador DERIVADO por módulo 11: cuerpo 14555666 ⇒ suma ponderada 155, resto 1, dv 11−1 = 10 = K.";
