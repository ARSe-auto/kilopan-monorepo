...
- [x] (P2) Escáner cámara full-screen + linterna + beep/vibración (zxing-js) como mejora progresiva; la captura manual sigue siendo el camino primario en iOS — cerrado 07-ago: `EscanerBulto.tsx` (mismo patrón que `EscanerTed.tsx`), botón agregado en `/cargar` junto a la captura manual, reusa `escanear()`; sin cámara sale `null` (degradación sin bloqueo). 7 tests de contrato `EscanerBulto.test.ts` — spec: `specs/kilopan/04-despacho-reparto.md` [AC-DES-07]
...
- [x] (P2) Validar el camino GATT contra una báscula real antes de darlo por bueno — descartado 08-ago junto con AC-PES-05: sin objeto validar contra hardware un camino que el equipo del cliente no puede ofrecer — spec: `specs/kilopan/02-catalogo-pesaje.md` [AC-PES-09]
- [x] (P2) UI de resolución de mermas al día siguiente — cerrado de verdad 08-ago: faltaba `estado_merma` en el SELECT de `/api/pesajes` (la pantalla filtraba contra undefined y nunca listaba nada); 4 e2e verdes — spec: `specs/kilopan/02-catalogo-pesaje.md` [AC-MERM-02]
...
- [x] (P2) [AC-PES-05] — descartado 08-ago (decisión del dueño): Web Bluetooth no existe en Safari/iOS, el equipo real del cliente nunca lo habría podido usar; `bascula.ts` eliminado del árbol — `specs/kilopan/02-catalogo-pesaje.md`
...
- [x] (P1-SEC) [AC-PES-08] — cerrado 08-ago (barrido supervisado): `e2e/pesar-outlier.spec.ts` (2 verdes) ejercita `confirmar_outlier` desde la pantalla y verifica por red que la confirmación reusa el MISMO sha256 sin refotografiar — `specs/kilopan/02-catalogo-pesaje.md`
...
- [x] (P1) [AC-ADM-01] — cerrado 08-ago (barrido supervisado): `e2e/administracion-usuarios.spec.ts` (3 verdes) prueba alta/edición por sus efectos (login real por equipo desechable: desactivar→401, PIN nuevo entra, candado de auto-desactivación, 403 a vendedor) — `specs/kilopan/10-administracion.md`
